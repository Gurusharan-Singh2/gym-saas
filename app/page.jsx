...
export default function HomePage() {
  ...
  const [demoSplitTab, setDemoSplitTab] = useState('chest');
  ...
  return (
    <div className="bg-[#09090B] text-[#FFFFFF] overflow-hidden">
      ...
      {/* 3.5. FEATURED DAY-WISE WORKOUT SPLIT & ANIMATED PERSON DEMOS */}
      <section className="py-20 bg-[#0A0A0D] border-t border-zinc-800">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col md:flex-row md:items-end justify-between mb-12">
            <SlideUp>
              <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-amber-500/10 border border-amber-500/30 text-amber-300 text-xs font-bold uppercase tracking-widest mb-3">
                ...
                <span>Day-Wise Hypertrophy System</span>
              </div>
              <h2 className="text-3xl sm:text-4xl lg:text-5xl font-extrabold font-display text-white">
                PRECISION 2-MUSCLE SPLIT &amp; ANIMATIONS
              </h2>
              <p className="text-zinc-400 text-xs sm:text-sm max-w-2xl mt-2 leading-relaxed">
                Experience evidence-based programming where every training day targets 2 opposing or synergistic muscle groups, backed by Beginner, Intermediate, and Hard exercise tiers with real-time animated biomechanical figures.
              </p>
            </SlideUp>

            <SlideUp delay={0.1} className="mt-6 md:mt-0">
              <Link href="/workouts">
                <MotionButton className="btn-gold px-6 py-3 rounded-xl text-xs uppercase tracking-wider font-extrabold flex items-center gap-2 shadow-lg">
                  <span>Explore All 7 Days &amp; 42 Exercises</span>
                  <ArrowRight className="w-4 h-4" />
                </MotionButton>
              </Link>
            </SlideUp>
          ...
          <div className="flex items-center gap-2 overflow-x-auto pb-2 mb-8 scrollbar-none">
            {[
              { id: 'chest', label: 'Day 1: Chest & Triceps', anim: 'bench_press', muscle: 'Chest' },
              { id: 'back', label: 'Day 2: Back & Biceps', anim: 'lat_pulldown', muscle: 'Back' },
              { id: 'legs', label: 'Day 4: Quads & Hamstrings', anim: 'squat', muscle: 'Quadriceps' },
              { id: 'biceps', label: 'Arm Focus: Bicep Curls', anim: 'bicep_curl', muscle: 'Biceps' },
            ].map((tab) => (
              <button
                key={tab.id}
                onClick={() => setDemoSplitTab(tab.id)}
          ...
          {/* Live Preview Showcase Cards */}
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center bg-[#141418] border border-zinc-800 rounded-3xl p-6 sm:p-8">
            <div className="lg:col-span-6 space-y-5">
              <div className="flex items-center gap-2.5">
                <span className="px-3 py-1 rounded-full bg-amber-500/10 border border-amber-500/30 text-amber-300 text-xs font-bold uppercase tracking-wider">
                  Live Biomechanical Demo
                </span>
                <span className="text-xs text-zinc-400 font-mono">60 FPS Articulated Figure</span>
              </div>

              <h3 className="text-2xl sm:text-3xl font-black font-display text-white">
                {demoSplitTab === 'chest' && 'Monday • Bench Press & Push Kinetics'}
                {demoSplitTab === 'back' && 'Tuesday • Lat Pulldowns & V-Taper Pull'}
                {demoSplitTab === 'legs' && 'Thursday • Olympic Squats & Lower Body Power'}
                {demoSplitTab === 'biceps' && 'Peak Contraction • Supinated Dumbbell Curls'}
              </h3>

              <p className="text-xs sm:text-sm text-zinc-400 leading-relaxed">
                {demoSplitTab === 'chest' &&
                  'Master the bar path, scapular depression, and 45-degree elbow tuck with live golden pectoral activation pulse.'}
                {demoSplitTab === 'back' &&
                  'Initiate pulling from latissimus dorsi depressors to build width and thickness without relying on forearm strain.'}
                {demoSplitTab === 'legs' &&
                  'Achieve deep parallel hip crease depth with upright thoracic posture, driving force directly through the mid-foot.'}
                {demoSplitTab === 'biceps' &&
                  'Isolate elbow flexion with strict dead-stop negatives to trigger maximum peak hypertrophy.'}
              </p>

              {/* 3 Difficulty Badges Preview */}
              <div className="grid grid-cols-3 gap-3 pt-2">
                <div className="p-3 rounded-xl bg-zinc-900 border border-zinc-800 text-center">
                  <div className="text-[10px] uppercase font-bold text-emerald-400 mb-0.5">Tier 1</div>
                  <div className="text-xs font-bold text-white">Beginner</div>
                  <div className="text-[10px] text-zinc-400 mt-1">Foundation Form</div>
                </div>
                <div className="p-3 rounded-xl bg-zinc-900 border border-zinc-800 text-center">
                  <div className="text-[10px] uppercase font-bold text-amber-300 mb-0.5">Tier 2</div>
                  <div className="text-xs font-bold text-white">Intermediate</div>
                  <div className="text-[10px] text-zinc-400 mt-1">Overload &amp; Volume</div>
                </div>
                <div className="p-3 rounded-xl bg-zinc-900 border border-zinc-800 text-center">
                  <div className="text-[10px] uppercase font-bold text-rose-400 mb-0.5">Tier 3</div>
                  <div className="text-xs font-bold text-white">Hard / Elite</div>
                  <div className="text-[10px] text-zinc-400 mt-1">Heavy Compound</div>
                </div>
              </div>

              <div className="pt-2">
                <Link href="/workouts">
                  <span className="text-xs font-bold uppercase tracking-wider text-white hover:text-amber-300 flex items-center gap-1.5 transition-colors">
                    <span>View All Exercises in Workout Split</span>
            ...
            {/* Live Interactive Biomechanical SVG Player */}
            <div className="lg:col-span-6">
              <div className="p-2 rounded-2xl bg-zinc-950/80 border border-zinc-800/80 shadow-2xl">
                <ExerciseAnimator
                  gifUrl={
                    demoSplitTab === 'chest'
                      ...
                      : 'https://raw.githubusercontent.com/hasaneyldrm/exercises-dataset/main/videos/0285-L1626nF.gif'
                  }
                  imageUrl={
                    demoSplitTab === 'chest'
                      ...
                      : 'https://raw.githubusercontent.com/hasaneyldrm/exercises-dataset/main/images/0285-L1626nF.jpg'
                  }
                  animationType={
                    demoSplitTab === 'chest'
                      ? 'bench_press'
